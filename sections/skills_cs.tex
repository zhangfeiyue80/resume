...
{Programming}
{C/C++,Python $\vert$ Gcc+make+shell $\vert$ Vim+tmux 
...
\begin{flushleft}\setstretch{1.25}
	...
	\SECTCONT{了解多进程， 多线程编程 }
	\SECTCONT{了解IPC(Inter-process communication)-管道/消息队列/信号量/共享内存/套接字}
	\SECTCONT{基于memcache分布式应用实现}
	\SECTCONT{基于postgres分布式应用实现}
	\SECTCONT{基于tuxedo中间件C/S应用程序开发}
	\SECTCONT{基于已有系统框架进行功能开发}
	%
	\SECTC{Network Development }
		{IP/TCP/UDP | HTTP/RTP, tcp连接建立释放，三次握手四次挥手}
		{了解IP/TCP协议, 路由, UDP/RTP ,音视频流协议, HTTP应用层协议}
